...
export class CryptoBufferProcessor implements IStoreProcessor<ArrayBuffer> {
	...
	private async getCryptoKey(): Promise<CryptoKey> {
		if (!this.key) {
			...
			this.key = await crypto.subtle.importKey('raw', digest, {length: 256, name: this.algorithm}, true, ['encrypt', 'decrypt']);
		}
		return this.key;
	}

	private async getBuffer(): Promise<ArrayBuffer | Uint8Array<ArrayBuffer>> {
	...
	private async encryptPromise(buffer: ArrayBuffer): Promise<ArrayBuffer> {
		...
		return mergeArrayBuffers(iv.buffer, await crypto.subtle.encrypt({iv, name: this.algorithm}, key, buffer));
	}
	...
	private async decryptPromise(buffer: ArrayBuffer): Promise<ArrayBuffer> {
		...
		return await crypto.subtle.decrypt({iv, name: this.algorithm}, key, buffer.slice(12));
	}
}
