...
export class TachyonExpireCache<Payload, Key extends string = string> extends EventEmitter<CacheEventsMap<Payload, Key>> implements IAsyncCache<Payload, Key> {
	...
	#cache = new Map<Key, CachePayload<Payload>>();
	#defaultExpireMs: undefined | number;
	...
	#buildFlatDataMap(): Map<Key, Payload> {
	...
	#handleRebuild(data: CacheMap<Payload, Key>) {
	 ...
	 */
	async #handleStore(limit: TachyonBandwidth): Promise<void> {
	...
	async #cleanExpired(limit: TachyonBandwidth): Promise<void> {
		const deleteData = this.#handleExpires();
		if (deleteData.size > 0) {
			this.#logMessage('cleanExpired', `expired count: ${deleteData.size.toString()}`);
			if (this.#testBandwidth(limit)) {
	...
	#notifyClear(deleteData: Map<Key, Payload>): void {
		this.emit('clear', deleteData);
	}

	#testBandwidth(limit: TachyonBandwidth): boolean {
